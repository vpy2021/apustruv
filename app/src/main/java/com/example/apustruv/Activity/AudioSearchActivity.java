...
import com.example.apustruv.AdapterClass.AudioSearchAdapter;
...
public class AudioSearchActivity extends AppCompatActivity {
    ...
    RecyclerView audioSearchList;
    List<AudioSearchModel>audioList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        audioSearchList = findViewById(R.id.audioSearchListID);

        audioDataAPICollection();

    }

    private void audioDataAPICollection() {
                ...
                try {
                    ...
                    if (generalArray.length() > 0) {
                        for (int i = 0; i < generalArray.length(); i++) {
                          ...
                          AudioSearchModel audioSearchModel = new AudioSearchModel();
                            audioSearchModel.setAudioTitleName(generalArray.getJSONObject(i).getString("title"));
                            audioSearchModel.setAudioImage(generalArray.getJSONObject(i).getString("post_url"));
                            audioList.add(audioSearchModel);

                        }

                        audioSearchList.setAdapter(new AudioSearchAdapter(getApplicationContext(), audioList));
                        audioSearchList.setLayoutManager(new LinearLayoutManager(getApplicationContext()));
